...
function features(parsers, feats, model)
    ...
    #########
    pvecs,dvecs,lvecs,rvecs,xvecs,xpvecs = postagv(model),deprelv(model),lcountv(model),rcountv(model),distancev(model),xpostagv(model)
    pvec0,dvec0,lvec0,rvec0,xvec0,xpvec0 = zeros(pvecs[1]),zeros(dvecs[1]),zeros(lvecs[1]),zeros(rvecs[1]),zeros(xvecs[1]),zeros(xpvecs[1])
    ...
    for p in parsers
	    ...
	    elseif fn == 'x'
                push!(fmatrix, a>0 ? xpvecs[s.xpostag[a]] : xpvec0)	
            elseif fn == 'p'
