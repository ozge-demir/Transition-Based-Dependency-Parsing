...
function makepmodel1(d; GPUFEATS=false)
    m = ([d["postagv"],d["deprelv"],d["lcountv"],d["rcountv"],d["distancev"],d["xpostagv"],d["parserv"]],
         [d["postago"],d["deprelo"],d["lcounto"],d["rcounto"],d["distanceo"],d["xpostago"],d["parsero"]])
...
function makepmodel2(o, s; ftype=Float32, intype=:normal)
    model = Any[]
    dpostag, ddeprel, dcount,dxpostag = o[:embed]
    for (k,n,d) in ((:postag,17,dpostag),(:deprel,37,ddeprel),(:lcount,10,dcount),(:rcount,10,dcount),(:distance,10,dcount),(:xpostag, length(s.vocab.xpostags), dxpostag))
        if intype == :normal
...
postagv(m)=m[1]; deprelv(m)=m[2]; lcountv(m)=m[3]; rcountv(m)=m[4]; distancev(m)=m[5]; parserv(m)=m[7]; xpostagv(m)=m[6]
